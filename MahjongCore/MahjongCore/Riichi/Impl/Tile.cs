...
namespace MahjongCore.Riichi
{
    ...
    #region TileType
        ...
        public static class TileTypeExtensionMethods
        {
            ...
            {
                RiichiGlobal.Assert(current.IsPlayer());
                RiichiGlobal.Assert(dealer.IsPlayer());
            ...
            {
                RiichiGlobal.Assert(seat.IsWind());
    ...
    public class ExtendedTile : ICloneable
    {
        public TileType Tile = TileType.None;
        public bool     Called = false;
        public bool     Reach = false;
        public bool     OpenReach = false;
        public bool     WinningTile = false;
        public Player   Caller = Player.None;
        ...
        public ExtendedTile()            { }
        public ExtendedTile(TileType tt) { Tile = tt; }

        public ExtendedTile(TileType tt, bool openReach)
        {
            Tile = tt;
            if (openReach) { OpenReach = true; }
            else           { Reach = true; }
        }
        ...
        public void Set(ExtendedTile et)
        {
            Tile = et.Tile;
            Called = et.Called;
            Caller = et.Caller;
            Reach = et.Reach;
            OpenReach = et.OpenReach;
            WinningTile = et.WinningTile;
        }

        public string GetHexString()
        {
            uint value = (uint)Tile.GetSkyValue();
            if (Reach)                    { value |= REACH_FLAG; }
            if (Called)                   { value |= CALLED_FLAG; }
            if (OpenReach)                { value |= OPEN_REACH_FLAG; }
            if (Caller == Player.Player1) { value |= CALLER_P1_FLAG; }
            if (Caller == Player.Player2) { value |= CALLER_P2_FLAG; }
            if (Caller == Player.Player3) { value |= CALLER_P3_FLAG; }
            if (Caller == Player.Player4) { value |= CALLER_P4_FLAG; }
            return string.Format("{0:X4", value);
        }

        public static bool TryGetExtendedTile(string value, out ExtendedTile et)
        {
            bool found = false;
            et = null;
            ...
            {
                int skyValue;
                if (int.TryParse(value, System.Globalization.NumberStyles.HexNumber, null, out skyValue))
                {
                    et = new ExtendedTile(TileTypeExtensionMethods.GetTile((int)(skyValue & TILE_MASK)));
                    et.Reach = (skyValue & REACH_FLAG) == REACH_FLAG;
                    et.Called = (skyValue & CALLED_FLAG) == CALLED_FLAG;
                    et.OpenReach = (skyValue & OPEN_REACH_FLAG) == OPEN_REACH_FLAG;
                    et.Caller = Player.None;

                    if ((skyValue & CALLER_P1_FLAG) == CALLER_P1_FLAG) { et.Caller = Player.Player1; }
                    if ((skyValue & CALLER_P2_FLAG) == CALLER_P2_FLAG) { et.Caller = Player.Player2; }
                    if ((skyValue & CALLER_P3_FLAG) == CALLER_P3_FLAG) { et.Caller = Player.Player3; }
                    if ((skyValue & CALLER_P4_FLAG) == CALLER_P4_FLAG) { et.Caller = Player.Player4; }
        ...
        public object Clone()
        {
            ExtendedTile et = new ExtendedTile();
            et.Set(this);
            return et;
        }
    }

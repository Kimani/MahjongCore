...
namespace MahjongCore.Riichi.Impl
{
    ...
    internal class GameStateImpl : IGameState
    {
        ...
        public void ExecutePostBreak_GameEnd()
        {
            ...
            GameComplete?.Invoke(new GameResultImpl(
                Settings,
                FirstDealer,
                Player1Hand.Score,
                Player2Hand.Score,
        ...
        private void Advance(PlayState nextMode, bool advancePlayer, bool skipCheck)
        {
            ...
            do
            {
                ...
                else
                {
                    ...
                    if (SkipAdvancePlayer)        { SkipAdvancePlayer = false; }
                    else if (queuedAdvancePlayer) { Current = Current.GetNext(); }
        ...
        private void QueryPostDiscardDecision(GameAction currAction, HandImpl hand, IPlayerAI ai)
        {
            if (currAction == GameAction.DecisionPending)
            {
                ...
                {
                    if (ai != null)                                                                                  { SubmitPostDiscard(ai.GetPostDiscardDecision(_PostDiscardInfoCache)); }
                    else if ((PreviousAction == GameAction.PromotedKan) || (PreviousAction == GameAction.ClosedKan)) { PostKanRequested?.Invoke(_PostDiscardInfoCache); }
                    else                                                                                             { PostDiscardRequested?.Invoke(_PostDiscardInfoCache); }
                }
                else
                {
                    _CachedPostDiscardPass.Reset();
        ...
        private bool CheckForPause(bool rewindHandler)
        {
            ...
            try
            {
                ExpectingPause = true;
                ShouldPause = false;
                if (rewindHandler) { PreCheckRewind?.Invoke(); }
                else               { PreCheckAdvance?.Invoke(); }
                pause = ShouldPause;
            }
        ...
        private ITile PickIntoPlayerHand(Player p, TileSource source, out ITile wallTile)
        {
            ...
            else { tileNumber = GetNextWallDrawSlot(); }
        ...
        // TODO: Move everything below to boardtemplateimpl...
        ...
        private void ProcessBoardOverride(IBoardTemplate template)
        {
                  ...
                  (sampleGame.State != PlayState.GameEnd));)          // Don't loop if the game is over.
            {
                ...
                void breakOnPrePickTile() { if (sampleGame.State == PlayState.PrePickTile) { sampleGame.Pause(); } }

                sampleGame.WallPicking += wallPickingPause;
                sampleGame.PostDiscardRequested += postRequested;
                sampleGame.PreCheckAdvance += breakOnPrePickTile;
